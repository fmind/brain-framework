...
FKF has one `fkf` command with verb-first subcommands. Terminal output defaults to text; piped output defaults to indented JSON. Override with global `--format text|json|jsonl` and select a base with `--base`.

The CLI help is authoritative and versioned with the package:
...
Every mutating path takes one fail-fast cross-process writer lock for the physical base. A concurrent writer exits operationally instead of waiting or interleaving changes. CLI `context` takes the lock because it saves its machine-local receipt snapshot; MCP and direct-service context reads do not. Other readers, `sync --dry-run`, `sync --preview`, `learn propose --dry-run`, `learn review`, `trust --check`, and `build [target] --check` stay lock-free.
...
Bare terms are the positional form of repeatable `--grep`; all terms must match. Record searches inspect scalar leaf values, never keys or container renderings. `--bodies` additionally searches only manifest-verified cached text and requires a term; it never invokes a body command. Repeatable `--where <field-path>=<value>` tests a bounded field path in records and does not apply to pages. The accepted path grammar is property and array traversal with an optional terminal `| length`; arbitrary jq expressions are rejected. `--limit` bounds records and pages separately, so a long record list cannot hide authored pages.
...
The bare command prints the resolved configuration plus override origins. `helpers` compares installed official helpers with the running package; `--refresh` restores drifted installed helpers and missing official helpers required by the base, writing each replacement atomically while leaving custom scripts untouched. `schema` prints the generated JSON Schema for editor completion and validation.
...
The stdio server is read-only and requires an explicit base. It exposes bounded `context`, `find`, `day`, `timeline`, `list`, `read`, and `graph` operations but no body execution. Pageable tools return strict opaque cursors bound to their arguments and exact result snapshot. See [MCP server](mcp.md).
...
FKF has no self-replacing `upgrade` command. Upgrade it through the package manager that owns its environment. A persistent uv tool installation also gives harness and schedule integrations a stable launcher; `uvx` is for one-shot commands.
