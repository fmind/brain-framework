...
A base can concentrate a lot of one person: mail headers, calendar attendees, ticket titles, optional shell-activity metadata, and coding-agent activity. The shipped shell collector is disabled by default and, when enabled, keeps time, directory, exit status, and duration, never command text. The session-metadata collector keeps identifiers, timestamps, workspace, harness, and model, never prompts or responses; the separately enabled session-trace importer writes bounded request and final-response excerpts into task pages. Network sources are also disabled by default. That concentration is the point, and it is also the risk. This page states what `fkf` actually defends against, by what mechanism, and where the defence stops.
...
The loader rejects every key it does not know, so a pull request cannot reintroduce `secrets:`, `env_file:`, `accounts:`, or `policy:`:

```text
fkf: invalid configuration: ~/brain/fkf.yaml: field secrets not found
```

All four fail that way and exit 2. No key replaces them: export the variable in the shell that launches FKF and let the provider CLI read it.

FKF configuration has no environment key. Provider-profile selectors such as `GH_CONFIG_DIR` or `CLOUDSDK_CONFIG` belong to the process that launches FKF, just like the provider CLI's credential state. This avoids committing machine/account selection to a shared base. A base that needs a reproducible selector can name a reviewed wrapper executable under its trust-digested `sources/` directory.

Two invariants are enforced mechanically rather than by review, because both are the kind of thing that erodes quietly:

1. No FKF application module imports a socket or HTTP client API. Dependencies may contain network clients, but stored-read code never calls one and MCP uses stdio only.
1. No non-comment Python source line names a credential-shaped `TOKEN`, `SECRET`, `PASSWORD`, `API_KEY`, or `PAT` value.
...
These details matter more than the flow:
...
- **Only explicit command surfaces execute what the base declares:** source `auth:` and `run:` during a non-dry-run `fkf sync` (including the write-free `--preview`), source hooks during `fkf test`, record `body:` during `fkf read --body`, and bounded `auth:` probes during explicit `fkf status --live`. Brief, MCP, and ordinary stored reads never execute a declared command. `?jq=` uses a bounded pure selector: a closed field-path grammar with an optional terminal `| length`, no process, filesystem, environment, or import access, and fixed expression, path-depth, and output limits. `fkf trust --check` reports the state and records nothing, while `--all` prints the full disclosure even when a concise change report is available.

## Why declared commands are argument lists

Every declared command crosses the process boundary as argv. The difference is which placeholders each one may use:
...
`auth:` takes no placeholder at all. It is literal argv, runs once per sync and only for a source that has due work, and its stdout and stderr are discarded rather than logged. A normal non-zero provider exit means "not ready" or `auth_required`; a signal, timeout, output overrun, unsafe path, or startup failure remains an operational error rather than being misreported as an authentication problem.

`run:` receives only dates and paths generated by FKF. The first item is a literal executable; FKF substitutes every later item independently and invokes it without a shell. Pipelines, globs, and expansion therefore require a helper under `sources/`, where the helper's shebang names its interpreter and trust covers the bytes and executable bit. No collected data chooses a run executable or argument.

`test:` receives only base and home paths. It has no collection window or stored field placeholder and writes no evidence. Its executable PATH begins with the base's trust-digested `tests/` tree; other declared command families never search that tree. A `tests/git` can intentionally shadow Git inside a hook, so the tree is reviewed executable code, not a generic repository-test directory and never a place for secrets. Trust detects changes; it does not sandbox a hook or withhold the provider environment.

`body:` fetches one record's body on demand, and its placeholders can come from data a provider returned. Before exec, each data-derived substituted value must be valid UTF-8, 1 to 256 bytes, contain visible content, contain no control or format characters, and not start with `-` or `@` — a leading `-` is read as an option by most CLIs, and a leading `@` as a response-file or file-upload selector by `curl` and `gh`. Unicode, spaces, and punctuation remain one opaque argument.

```text
fkf: refusing to run body: for source jira-issues: the id value "--help" is not a safe opaque argv value (valid UTF-8, 1..256 bytes, no leading '-' or '@', controls, or invisible format characters)
```

Refusing before execution is the entire guarantee. There is no shell, so there is no quoting to get right. With `bodies: none`, a fetched body is printed and not stored; `cache` and `sync` instead write it to the ignored, manifest-verified cache described below. Entity URIs remain offline graph nodes and never select executable lookup logic.

## Paths are confined, and escapes are rejected

Every base-relative path — a [URI](uris-graph.md) you type, a link inside a wiki page, a layer path built by the store — is normalised through one function that refuses absolute paths, `~` prefixes, backslashes, NUL bytes, and anything that resolves outside the base.
...
Directories are created `0700`, ordinary files `0600`, and executable helpers, clients, and hooks under `sources/`, `clients/`, and `tests/` `0700`. Nothing in a base is world-readable by construction, because the whole directory is personal data by construction.
...
Every mutating CLI path also takes one fail-fast cross-process advisory lock keyed by the physical base path and stored outside the base. Symlink aliases therefore contend on the same writer identity. FKF refuses lock acquisition when neither a usable absolute home nor XDG state root exists, or when the lock-state directory resolves inside the base; the owner-only lock directory and persistent lock file stay outside cloneable content. A second writer fails immediately with an operational error; it never waits and never risks interleaving two multi-file operations. Read-only commands, `sync --dry-run`, `sync --preview`, `trust --check`, and `build [target] --check` do not take the lock. The lock coordinates FKF writers, not arbitrary editors or git processes, so atomic replacement and cache snapshot validation remain necessary.

Learning proposals are ignored machine-local unified diffs under `.agents/tmp/learn/`, not knowledge. Review is read-only. Apply accepts only flat wiki and project targets, snapshots affected authored and derived files, validates the resulting layers strictly, and rebuilds caches before archiving the proposal. A mismatch or failure restores the snapshot, so a half-applied proposal cannot become durable evidence.

Reads are bounded: 1 MiB for configuration and each trusted file under `sources/`, `clients/`, or `tests/`; 4 MiB for a Markdown page or cached body; 8 MiB for body-cache manifests; 64 MiB for a collected document; and 64 MiB per captured subprocess stream. The rebuildable body cache is additionally capped at 4,096 entries and 512 MiB. A command that overruns either stream limit fails its day rather than being truncated into a document that parses and lies. Every child starts in its own process group; cancellation, timeout, or either stream overrun kills and waits for that group so descendants cannot outlive the failed command.

`fkf status` diagnoses unsafe filesystem modes and prints a precise owner-only repair command; it never changes the base. The command excludes `.git`, sets base directories to `0700` and ordinary files to `0600`, and preserves executable intent under `sources/`, `clients/`, and `tests/` while removing group and other access:
...
The omitted tail applies `0600` to files outside the three execution trees, then walks every file below `sources/`, `clients/`, and `tests/` and applies `0700` only when that file is already executable, otherwise `0600`. `fkf status` prints the full directly runnable command; the shortened form above only keeps this page readable.
...
- **Trust answers "did fkf's execution definition change", not "is this safe".** The gate proves you read the command definitions and hashed `sources/` and `tests/` trees and that they have not changed since. It does not sandbox them: a trusted command runs with your privileges and can deliberately interpret any local file. Authored and collected layers are data, not part of the digest; sourcing one from a command turns mutable data into code outside the guarantee.
...
- **Stored reads and MCP make no network request; explicit execution paths may.** During a non-dry-run `fkf sync`, `fkf test`, an explicit trust-gated `fkf read --body`, or an `auth:` probe from `fkf status --live`, a provider command can talk to its provider under its own credentials and audit logs. Live status discards probe output and collects nothing. Sources with `bodies: sync` prefetch only after their evidence write; failures remain visible and retryable. `bodies: cache` writes only after an explicit body read, while the default `none` never stores the text. Installation and upgrades are external package-manager and package-index actions that touch no base. `brief`, `context`, `find`, `graph`, MCP, and `read` without `--body` touch only files; cached bodies remain untrusted data.
- **No telemetry, no account, no daemon, and no native Windows.** WSL2 works — it is Linux — provided the base lives on the Linux filesystem rather than an NTFS `/mnt/c` mount, where the owner-only modes `fkf status` checks mean nothing. CI verifies the Python package on Linux and macOS runners; releases contain an architecture-neutral Python wheel and source distribution, not native architecture archives.
