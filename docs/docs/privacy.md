...
Retrieval opens the published index file read-only in place after checking that its manifest still matches the current notes, records and configuration; a replaced or damaged file falls back to the in-memory path. Back up durable notes, records, configuration and adapters. SQLite is disposable. Keep private bases and recovery archives under appropriate owner-only permissions and encryption. Rehearse restore, rebuild and exact evidence reads; Git alone is insufficient when collected evidence is ignored.
