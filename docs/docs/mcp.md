...
The server opens no socket and no port. The client launches the installed `fkf` command as a child process and speaks JSON-RPC over its standard input and output; when the client disconnects, the process ends. There is nothing to start at boot, nothing to leave running, and nothing to authenticate against.
...
## Why `--base` is required { #why---base-is-required }
...
Every record and page in every result carries the `uri` that addresses it, which is what makes the tools compose: `context` or `find` finds candidates, `read` opens one, `graph` asks what else touches it. The grammar is the same one described in [URIs and the graph](uris-graph.md).
...
Before pagination, an MCP `list`, directory `read`, projects resource, or tag resource may examine at most 10,000 filesystem entries, retain 10,000 items, read 64 MiB of source files, and encode 64 MiB of complete-result snapshot metadata. Those ceilings make the snapshot digest and every continuation restart-stable without letting a deliberately huge base consume unbounded memory or work. An over-limit request fails with the breached ceiling and suggests a narrower query, child directory, or exhaustive local CLI command; it never returns a partial listing. These are MCP disclosure limits only: CLI listings and graph behavior are unchanged.
...
This is the same reasoning as the token budget in [context](context.md): an agent that can pull an unbounded listing will, and the cost lands in a context window rather than on disk. Pagination makes completeness possible when it is genuinely needed; `context` with a budget or a narrower filter remains the better first question.
