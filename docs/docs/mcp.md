...
```bash
bf mcp                 # every registered brain, or the enclosing one
bf mcp --brain brain    # one brain
```
...
In your host's MCP settings, create a stdio server with command `bf` and arguments `mcp`, `--brain`, `brain` (replace `brain` with the intended registered brain). Use the full executable path from `command -v bf` if the host does not inherit your shell's PATH. The host starts and stops the process; you do not need a separate daemon or network port.

The selected brains are fixed when the server starts. Restart it after changing registrations or updating Brain Framework. Search covers those selected brains; the `brain` argument on `read` disambiguates a returned ref, and cannot open a brain outside that selection.
