...
`fkf find` with `--layer wiki` is an exhaustive lexical scan over the slug, title, description, tags, explicit relations, and body. A few hundred pages is a scan, and a scan remains inspectable — the same reasoning that governs [context packs](context.md).
...
The separate root `skills/` catalog is owner-authored and never refreshed from package assets. Reviewed packages there can be linked into the user's shared harness catalog with `fkf skills install`. They are procedures, not OKF knowledge or retrieved evidence, and their names must be unique across every installed base.
