...
# Give your coding agent the missing why
...
1. **An open graph over URIs.** Every record and page has a relative URI; a base may declare any non-reserved lowercase entity scheme and shared relation field. The root-level `graphs/src.tsv` transcribes those declarations and authored links.
...
- [Getting started](getting-started.md) — install FKF, generate a demo base, then collect your first real day.
- [The base and fkf.yaml](base.md) — understand the five layers, discovery, overrides, defaults, and trust inputs.
- [Configuration schema](schema.md) — define shared field meanings and bind an editor to the generated JSON Schema.
- [Sources are commands](sources.md) — compose direct argv and helpers, validate previews, and control body fetching.
- [URIs and the graph](uris-graph.md) — use one address grammar and follow only declared relationships.
- [Context and receipts](context.md) — see the closed ranking arithmetic, token budget, and reproducibility receipt.
- [Command reference](commands.md) — find every command and its write or execution boundary.
- [MCP server](mcp.md) and [agent harnesses](harnesses.md) — connect a base to an agent through bounded offline reads.
- [Privacy and security](privacy.md) — understand exactly what the trust and data boundaries protect.
- [The wiki format](okf.md) — write flat, tagged OKF v0.2 pages for durable knowledge.
