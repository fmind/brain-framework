...
The last two transcribe a declared alias rather than a link: each records one `same-as` edge from the alias to its canonical URI, so an identity merge stays auditable under `--kind same-as`. [Declared identities](identities.md) has the alias grammar.
