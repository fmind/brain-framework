...
This repository builds **`fkf`**: one typed Python package, one console command, and three bundled skills. A base is a git repository of JSON and Markdown that FKF collects, links as URIs, and retrieves for coding agents. `README.md` explains the product; this file defines the contributor contract.
...
1. Keep tests hermetic: use a temporary `HOME`, unset `FKF_BASE`, use synthetic fixtures, and replace every provider-backed declared command through the `Runner` seam or a fake executable on a temporary `PATH`. No test makes a network or provider call. Local POSIX tools such as `sh`, `bash`, `python3`, `jq`, `sqlite3`, and `git` may run for real only where they are the subject under test: the process and source execution boundary, shipped helpers, the synthetic relative-base helper regression, and repository invariant or status-policy checks.

## Product boundaries

- **One package and command.** The root distribution is `fkf`; do not add a nested project, plugin, provider SDK, or second shipped command. Zensical is a development-only documentation dependency, not product code.
...
- **Two skill scopes.** `.agents/skills/` holds bundled and base-specific packages. Root `skills/` holds optional owner-authored packages. Explicit `fkf skills install` links them into a real user catalog outside source repositories; no privacy prefix is required. Names are unique across all installed skills and collisions fail before writes. Initialization and harness setup never install user-scope skills.
...
- **Flat role-based paths.** Durable collected state lives in `inventories/`; its permanent v1 evidence marker remains `layer: "index"`. Generated graph artifacts live in `graphs/`, lexical artifacts in `indexes/`, and base-owned maintenance in `operations/`. Public configuration and layer names use `inventories`; only the evidence codec maps the stored marker.
...
- **Small, typed Python.** Keep cancellation explicit, parse external values into strict boundary types, split natural seams rather than line counts, and prefer a pure helper to a framework.

## Trust and execution

- **Stored reads are offline.** `brief`, `context`, `find`, `graph`, MCP, and `read` without `--body` execute no command or network request. Context and `find --bodies` may read manifest-verified text from the ignored body cache but never fetch a miss. Explicit `status --live` may run only trusted `auth:` probes and never collects evidence. The `?jq=` compatibility spelling accepts only the closed field-path grammar and optional terminal `| length`; it has no code, environment, file, input, or import access, and its result is bounded.
...
- **Commands stay explicit.** Prefer direct provider argv, then a `.sh` or `.py` collection/body helper under `<base>/sources/`; put base-owned source verification hooks under `<base>/tests/`. Declared commands run from `/`; use `{{base}}` for data paths, and declare bare executable names and non-standard interpreters in `requires:`.
- **Trust covers execution.** Hash the canonical plan from both config files plus every entry and executable bit under `<base>/sources/`, `<base>/clients/`, and `<base>/tests/`; refuse symlinks. Execution-affecting changes re-arm trust. Never source authored or collected layers. Trust is disclosure and change detection, not a sandbox.
...
- Every mutating CLI path takes one fail-fast lock per physical base, including symlink aliases. CLI `context --save-receipt` takes it to save a snapshot; ordinary CLI, MCP, and direct-service context reads do not. Other readers, dry runs, previews, `trust --check`, and `build [target] --check` remain lock-free.

## Derived content and retrieval

- **Generated Markdown stays inside marked blocks.** `build wiki` owns one block in `wiki/index.md` through `src/fkf/marked_block.py`. `build` and `build all` update the wiki block before the graph and lexical index because the exact page bytes are input to both caches.
- **Files own links; the graph is a cache.** Record edges come from stored `relation: true` fields. Markdown edges come from links, tags, and declared relation fields under `relations:`. Explicit `identities:` declarations and supported authored `aliases:` canonicalize exact URI aliases and emit provenance-bearing `same-as` edges. Human names may resolve a query, but never create graph edges. Do not infer relationships from titles, prose, arbitrary frontmatter, or fuzzy identity matching.
...
- **The lexical index is only a cache.** `indexes/` is ignored, digest-bound to searchable evidence and ranking semantics, and rebuilt after searchable writes change. It supplies conservative candidates and term statistics only; Python still scores durable evidence. Missing, stale, or corrupt cache bytes make `context` and lexical `find` scan and name the fallback in their receipt.
...
- **MCP is read-only and bounded.** It requires `--base`, omits `--body` and the CLI-only Git tracking audit, and pages at 100 items with opaque cursors bound to arguments and an exact snapshot. Pre-pagination listing and directory reads cap examined entries and retained items at 10,000 each, cumulative source bytes at 64 MiB, and retained snapshot metadata at 64 MiB; crossing a cap fails the complete call. Text and structured results carry the same compact JSON and stay within 4 MiB. The wiki-index resource keeps its curated body. Client errors expose only base-relative paths and anonymize home and FKF state paths.

## Code map

| Path                                 | Responsibility                                                                                  |
| ------------------------------------ | ----------------------------------------------------------------------------------------------- |
| `src/fkf/cli*.py`                    | Typer commands, output envelopes, text renderers, and exit codes 0, 1, 2, 3, and 130.           |
| `src/fkf/config.py`                  | Strict configuration, schema, overlays, durations, and source declarations.                     |
| `src/fkf/store.py`                   | Published-path confinement, bounded I/O, atomic writes, physical-base locking, and base access. |
| `src/fkf/process.py`                 | Direct-argv execution, environment isolation, process groups, output bounds, retry, and pacing. |
| `src/fkf/{documents,sync}.py`        | Field projection, durable documents, collection orchestration, bodies, and rebuild sequencing.  |
| `src/fkf/{graph,lexical,context}.py` | Digest-bound caches and deterministic offline retrieval.                                        |
| `src/fkf/mcp*.py`                    | Read-only MCP tools, resources, cursors, privacy, and bounded responses.                        |
| `src/fkf/private_skills.py`          | Conflict-safe publication of base-owned private skills into the user catalog.                   |
| `src/fkf/assets_data/`               | Bundled presets, helpers, evaluations, and three skills; edit these sources, not base copies.   |
| `tests/`                             | Hermetic behavior, repository invariants, assets, packaging, and differential regressions.      |
| `docs/`                              | Zensical source pages, exact site-license copies, and generated `fkf.schema.json`.              |

## Verification

`mise run all` runs formatting, static checks, hermetic branch-coverage tests, the coverage ratchet, and distribution build/install smoke tests. Lefthook and CI use the same tasks. Use `mise run generate:schema` after loader changes; do not hand-edit generated schema. A runtime dependency change also updates `THIRD_PARTY_NOTICES.md` and its completeness test.

Tests own the exact CLI vocabulary, URI grammar, generated artifacts, schema parity, preset fixtures, trust and storage boundaries, complete third-party notices, and closed-vocabulary retrieval scoring. Large graph observations are opt-in tests, not thresholds. There is no source-of-truth database; derived caches must be rebuildable by `fkf build`, digest-bound to their inputs, and git-ignored.
