...
1. Set the same version in `pyproject.toml` and `src/fkf/__init__.py`, refresh `uv.lock` if needed, and add a dated `## [vX.Y.Z]` section in `CHANGELOG.md`. Document breaking changes and update pinned installation examples. `scripts/release-notes vX.Y.Z` must return only that section.
...
Keep private-base details out of release notes and repository proposals. Record future release lessons here only when they change a concrete step.
