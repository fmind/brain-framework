...
`fkf` returns evidence for the agent to interpret: project state, decisions, next actions and collected source items. Output is JSON. Select the intended audience first: `--base NAME|PATH`, then `FKF_BASE`, then the enclosing base, then every registered base. Use an explicit base for work context.
...
fkf read git-commits:team/archive@abc1234   # one illustrative record ref
```
...
1. Preserve returned refs literally and quote them in shell commands. A `#` inside a record ID is part of its identity; note section refs use the heading after the `.md` filename. Use `--base NAME` when that base must be present or when limiting context to a team's audience.
...
Search and read never collect. If a question needs missing or newer evidence, report the gap; collection requires the user's authorization. Answer with the conclusion, supporting refs and any material uncertainty. FKF does not generate the answer or verify a source's claims.
