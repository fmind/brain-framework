...
1. **Stale notes**: `bf read projects` marks active or blocked projects with `review` when their note is older than 14 days or newer items link to them (`new_links`); read the project for those backlinks, then refresh the note or set its verified current status.
