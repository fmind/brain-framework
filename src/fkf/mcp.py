...
from fkf.models import Error, Query, Status, encode, explain, moment
...
def server(stores: list[Store]) -> MCPServer:
    ...
    def reply(operation: Callable[[], dict[str, object]]) -> CallToolResult:
        try:
            ...
            for root in roots:
                message = message.replace(root, "<base>")
