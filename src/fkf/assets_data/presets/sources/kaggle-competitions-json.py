#!/usr/bin/env python3
"""Validate and project the complete Kaggle competitions inventory."""
...
def project(_arguments: list[str]) -> list[dict[str, Any]]:
    values = invoke(['kaggle-json.py', '--all-pages', 'competitions', 'list', '--group', 'entered', '--format', 'json'])
    ...
    for item in values:
        ...
        if not isinstance(ref, str) or not ref or ref in seen:
            raise ValueError
        seen.add(ref)
        record = {key: item.get(key) for key in ('ref', 'deadline', 'category', 'reward', 'teamCount', 'userHasEntered', 'userRank')}
...
def main(arguments: list[str]) -> int:
    if arguments[:1] in (["--version"], ["-v"]):
        sys.stdout.write("kaggle-competitions-json.py (fkf preset helper)\n")
        return 0
    if arguments:
        sys.stderr.write("usage: kaggle-competitions-json.py\n")
        return 2
    try:
        ...
        sys.stderr.write("kaggle-competitions-json.py: provider or response validation failed; no output emitted\n")
        return 1
