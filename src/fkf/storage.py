"""Confined, bounded filesystem access and one physical-base writer lock."""
...
def relative(value: str) -> tuple[str, ...]:
    ...
    if path.is_absolute() or any(p in {"", ".", ".."} for p in parts) or "\x00" in value or "\\" in value:
        raise Error("expected a normalized base-relative path")
    return tuple(parts)
...
class Store:
    ...
    def __init__(self, root: Path) -> None:
        try:
            ...
            raise Error("base directory does not exist; pass an existing --base or FKF_BASE") from error
        if not self.root.is_dir():
            raise Error("base must be a directory")
...
def state_store(root: Path) -> Store:
    """Private runtime state stays outside the evidence base."""
    ...
    if not home.is_absolute() or home.resolve().is_relative_to(root):
        raise Error("state directory must be absolute and outside the base")
    directory = home / "fkf" / digest(os.fsencode(root))
