...
class Source(Model):
    ...
    enabled: bool = True
    # A window source upserts the items it returns; a snapshot source replaces its complete catalog.
    ...
    # Zero keeps collection manual. These are due rules for `fkf update`, not an installed schedule.
...
class Config(Model):
    """One base: its name and the sources it may collect."""
...
class Registration(Model):
    ...
    # Collection trust lives outside the base: a cloned or shared base never runs its sources by itself.
    collect: bool = False


class UserConfig(Model):
    """The bases this user searches by default, and which of them may run collectors on this machine."""

    bases: dict[Annotated[str, Field(pattern=NAME)], Registration] = Field(default_factory=dict)
